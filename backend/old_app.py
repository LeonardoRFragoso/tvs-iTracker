...
# Registros modulares (rotas públicas/SPAs, service worker, middleware de monitoramento,
# rotas de monitoramento e handlers de WebSocket)
...
# =========================
# Estados globais (modularizados)
# =========================
from monitoring.state import (
    TRAFFIC_STATS, TRAFFIC_MINUTE, TRAFFIC_LOCK, UPLOAD_METRICS, SYSTEM_NET_LAST
)
from realtime.state import (
    CONNECTED_PLAYERS, SOCKET_SID_TO_PLAYER, SOCKET_SID_TO_USER, PLAYER_PLAYBACK_STATUS
)
...
def _measure_upload_latency_start():
    ...
    try:
        return None  # Desativado aqui; versão modular em monitoring.middleware
    except Exception:
        pass
...
def _track_upload_traffic(response):
    ...
    return response  # Desativado aqui; versão modular em monitoring.middleware

# =========================
# WebSocket events
# =========================

@socketio.on('connect', namespace='/legacy_disabled')
...
@socketio.on('disconnect', namespace='/legacy_disabled')
...
@socketio.on('join_player', namespace='/legacy_disabled')
...
@socketio.on('join_admin', namespace='/legacy_disabled')
...
@socketio.on('player_sync_request', namespace='/legacy_disabled')
...
@socketio.on('distribution_status_update', namespace='/legacy_disabled')
...
@socketio.on('player_heartbeat', namespace='/legacy_disabled')
...
@socketio.on('request_content_download', namespace='/legacy_disabled')
...
@socketio.on('playback_event', namespace='/legacy_disabled')
...
@socketio.on('player_command', namespace='/legacy_disabled')
...
@app.route('/__legacy_disabled__/api/<path:any_path>', methods=['OPTIONS'])
def api_cors_preflight(any_path):
    return make_response('', 204)

# Servir arquivos de upload
@app.route('/__legacy_disabled__/uploads/<path:filename>')
...
@app.route('/__legacy_disabled__/tizen-player/<path:filename>')
...
@app.route('/__legacy_disabled__/sw.js')
@app.route('/app/__legacy_disabled__/sw.js')
...
@app.route('/__legacy_disabled__/api/system/network-info')
...
@app.route('/__legacy_disabled__/api/public/companies', methods=['GET'])
...
@app.route('/__legacy_disabled__/k/<code>')
...
@app.route('/__legacy_disabled__/')
def root_index():
    return _kiosk_landing_html()

# Admin SPA
@app.route('/__legacy_disabled__/app')
@app.route('/__legacy_disabled__/app/<path:path>')
...
# TV landing dedicada
@app.route('/__legacy_disabled__/tv')
def tv_landing():
    return _kiosk_landing_html()

# Player page no SPA
@app.route('/__legacy_disabled__/kiosk/player/<player_id>')
...
@app.route('/__legacy_disabled__/<code:code>')
...
@app.route('/__legacy_disabled__/<path:admin_route>')
...
@app.route('/__legacy_disabled__/static/<path:filename>')
@app.route('/app/__legacy_disabled__/static/<path:filename>')
...
@app.route('/__legacy_disabled__/manifest.json')
@app.route('/app/__legacy_disabled__/manifest.json')
def serve_manifest():
    return _serve_react_file('manifest.json')

@app.route('/__legacy_disabled__/favicon.ico')
@app.route('/app/__legacy_disabled__/favicon.ico')
def serve_favicon():
    return _serve_react_file('favicon.ico')

@app.route('/app/__legacy_disabled__/asset-manifest.json')
...
@app.route('/__legacy_disabled__/api/monitor/traffic', methods=['GET'])
@jwt_required()
def monitor_traffic():
...
@app.route('/__legacy_disabled__/api/monitor/players', methods=['GET'])
@jwt_required()
def monitor_players():
...
@app.route('/__legacy_disabled__/api/monitor/traffic/timeseries', methods=['GET'])
@jwt_required()
...
@app.route('/__legacy_disabled__/api/monitor/traffic/top', methods=['GET'])
@jwt_required()
...
@socketio.on('join_admin_room', namespace='/legacy_disabled')
...
@socketio.on('leave_admin_room', namespace='/legacy_disabled')
...
@app.route('/__legacy_disabled__/api/system', methods=['GET'])
@app.route('/__legacy_disabled__/api/monitor/system', methods=['GET'])
@jwt_required()
...
def emit_system_stats_job():
    # Mantido por compatibilidade; jobs reais estão em monitoring.jobs
    try:
...
@app.route('/__legacy_disabled__/api/monitor/traffic/accumulated', methods=['GET'])
@jwt_required()
def monitor_traffic_accumulated_legacy():
